...
class RegionService extends Service {
  ...
  async rankByPOR(data) {
    const res = await this.app.mysql.query('select region.Name,sum(TotalGrossIncome) from transactions,salesperson,store,region where Status = \'Finished\' and SalespersonName = salesperson.Name and StoreAssigned = store.StoreID and store.Region = region.RegionID group by region.Name');
    if (!res) {
      return {
        success: false,
        errno: 1011,
  ...
  async rankByVOR(data) {
    const res = await this.app.mysql.query('select region.Name,sum(NumberOfProducts) from transactions,salesperson,store,region where Status = \'Finished\' and SalespersonName = salesperson.Name and StoreAssigned = store.StoreID and store.Region = region.RegionID group by region.Name');
    if (!res) {
      return {
        success: false,
        errno: 1011,
